...
```{r}
df_inhib_combined %>% 
  ggplot(aes(BoutLatency, color = Type)) + geom_density() + xlim(-10,80)
```
...
But here we don't whales outside of their own group or in a different order. (maybe Riccardo can figure out what he did here)
```{r}
df_whale <- read_csv(here("Data", "PilotWhales_jensen.csv"))
rm(OverlappingBouts)
